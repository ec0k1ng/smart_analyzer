...
- 已将“分析后曲线页首次显示不完整”从当前阻塞中收口，真实机器回归确认该问题已解决。
- 数据加载链路进一步统一到“按当前分析真正需要的接口映射信号处理”的策略，非必要列不再进入后续分析处理。
- 接口映射表中 time_s 的系统说明文案已收敛为“时间轴，单位 s”，固定首行逻辑继续保留在程序行为中，不再写进说明文本。
