...
- 曲线页在用户切换数据文件后会自动执行最佳缩放，避免上一份数据的横纵轴范围残留到新图表。
- 主页按钮区新增“打开报告目录”，位置移动到“开始分析”左侧，减少用户在配置页和主页之间来回跳转。
- 项目发布名称统一调整为 Apex Automata Insight Studio，GUI 标题、README、入口文档、模板提示词与打包元数据已同步。
- outputs/、outputs_batch/、outputs_real/ 下的导出产物以及 src/tcs_smart_analyzer.egg-info 下的生成元数据已纳入仓库噪音清理范围。

## 2026-05-03

### 报告与模板工作台收口

- DAT、MDF、MF4 已补上与 CSV/Excel 一致的探测后按需读取路径，减少无关列/通道读取。
- BLF/ASC 详细日志已明确展示按需 DBC 解码行为，避免误解为全量总线信号展开。
- 曲线页在无分析结果时也允许保留和编辑信号配置，仅显示占位内容。
- 模板工作台已支持草稿模板创建、未保存标记、Ctrl+S 保存、Jinja 语法校验、打开模板目录。
- HTML 默认模板、demo 模板和示例模板已统一为左侧固定目录 + 分组/文件层级 + 曲线截图结构。
- Word 报告首页目录已改为真实内容，并通过内部 hyperlink + bookmark 支持可点击跳转。
- Word 目录项样式已去掉下划线，文件级锚点已缩短为安全格式，修复数据名称跳回标题行的问题。

### 文档与发布前整理

- HANDOFF、PROJECT_STATUS、VALIDATION、TODO、README 已同步到当前代码事实。
- 删除根目录失败残留文件 pytest_exporters_failure.txt，减少发布噪音。
