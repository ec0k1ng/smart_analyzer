# Apex Automata Insight Studio 交接说明
...
- GUI / CLI 对外命令名分别为 apex-automata-insight-studio 和 apex-automata-insight-cli。
...
- 曲线页在切换数据文件后会自动执行最佳缩放，避免沿用上一份数据的轴范围。
...
- outputs/、outputs_batch/、outputs_real/ 下的导出产物与 src/tcs_smart_analyzer.egg-info 下的生成元数据应继续视为发布噪音，不应再纳入版本控制。
