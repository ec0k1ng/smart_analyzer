...
项目当前处于“主闭环可用、最近一轮聚焦回归已通过、接口映射与打滑量算法已升级、但真实桌面环境曲线页仍建议最终手测确认”的阶段。
...
- GUI 离屏冒烟脚本仍保留 chart_plot_area_full_enough 等探针，但当前未重新刷到整套通过结论。
- 本轮新增的局部定向离屏验证已经通过：子框信号 Ctrl+W 局部隐藏/显示、hidden_signals 持久化、接口映射真实信号列可缩减到 1 列。
