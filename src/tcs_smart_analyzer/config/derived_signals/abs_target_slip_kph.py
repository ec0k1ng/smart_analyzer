...
DERIVED_SIGNAL_DEFINITION = {
    ...
    "description": "全文件 ABS 激活阶段的时间加权平均负打滑量，作为 ABS 全局目标打滑量单值。",
    "algorithm_summary": "对所有 ABS 激活区间内的 -|slip_kph| 按时间差做加权平均，输出负值目标打滑量标量。",
}
...
def calculate_signal(dataframe):
    abs_columns = ["abs_active_fl", "abs_active_fr", "abs_active_rl", "abs_active_rr"]
    active_columns = [column_name for column_name in abs_columns if column_name in dataframe.columns]
    if not active_columns or "slip_kph" not in dataframe.columns or "time_s" not in dataframe.columns:
        return float("nan")

    abs_active = dataframe[active_columns].any(axis=1).astype(bool)
    if not abs_active.any():
        return float("nan")
    ...
    valid_mask = abs_active & slip.notna() & (time_diffs > 0)
    if valid_mask.any():
        return float(np.average(slip[valid_mask], weights=time_diffs[valid_mask]))
    fallback_mask = abs_active & slip.notna()
