...
KPI_DEFINITION = {
    ...
    "derived_inputs": ["slip_kph", "tcs_target_slip_kph"],
    "trend_source": "tcs_ctrl_time_max_s",
    "unit": "s",
    "description": "TCS 单次激活后，从激活开始到绝对打滑量稳定进入目标带内的最大时长。",
    "algorithm_summary": "逐段扫描 TCS 激活区间，比较 |slip_kph| 与目标打滑量加容差的稳定带，找到首次持续保持稳定的时刻作为该段控滑时间，再取各段最大值。",
    "threshold": 1.5,
    "source": "TCS标定规范：控滑过程应在1.5秒内完成",
    "pass_condition": "value <= threshold",
    "rule_description": "所有 TCS 激活事件中，控滑时间最大值应不超过 1.5 s。",
    "pass_message": "TCS 最大控滑时间达标。",
    "fail_message": "TCS 最大控滑时间超标。",
}
...
def calculate_kpi_series(dataframe):
    target_column = "tcs_target_slip_kph"
    ...
    active_columns = [column_name for column_name in ["tcs_active_fl", "tcs_active_fr", "tcs_active_rl", "tcs_active_rr"] if column_name in dataframe.columns]
